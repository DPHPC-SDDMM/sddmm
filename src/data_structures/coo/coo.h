...
namespace SDDMM{
    ...
    public:
        ...
        */
        COO static generate(
                vec_size_t n, vec_size_t m, float sparsity,
            ...
            )
        {
            ...
            auto nr_elements = static_cast<vec_size_t>(total_elements * sparsity);
            ...
            std::uniform_int_distribution<vec_size_t> row_distribution(0, n-1);
            std::uniform_int_distribution<vec_size_t> column_distribution(0, m-1);
            ...
            std::map<vec_size_t, vec_size_t> row_to_column;

            auto elements_remaining = nr_elements;

            while (elements_remaining)
            {
                vec_size_t row = row_distribution(generator);
                vec_size_t column = column_distribution(generator);
